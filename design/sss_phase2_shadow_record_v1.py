"""HERMES shared-stream recovery Phase-2 — SHADOW DECISION RECORD (INERT / DESIGN-ONLY).
...
STATUS: INERT / NOT WIRED. Imported by NO runtime path. A DESIGN artefact only.

A ShadowDecisionRecord is the immutable, versioned, offline-replayable record of ONE shadow evaluation:
  (evidence snapshot) -> core.decide() -> DecisionEnvelope -> comparison-vs-current-authority.

It is MECHANICALLY impossible to mistake for a reconnect command:
  * shadow_only is ALWAYS true and consumer_live is ALWAYS false (constants, validated);
...
snapshot alone. No secrets, UTC only.
"""
...
class CurrentAuthorityObservation:
    ...
    way that changes its ordering or timing. `evaluated=False` means the current authority did not run a recovery
    evaluation for this cycle (e.g. no per-instrument sustained-red pass), which yields CURRENT_AUTHORITY_NOT_EVALUATED.
    """
    evaluated: bool
