"""HERMES shared-stream recovery Phase-2 — EVIDENCE SNAPSHOT model (INERT / DESIGN-ONLY).
...
Base: canonical main 8355044b (Phase-1 pure core merged at utils/hermes_shared_stream_recovery_v1.py).

STATUS: INERT / NOT WIRED / NOT FOR MERGE-BY-IMPLEMENTATION-WO. This module is imported by NO runtime path
(main.py / utils/watchdog.py / adapters / provider client / any runner / docker-compose / Dockerfile / cron /
systemd). It is a DESIGN artefact: an immutable, versioned, provenance-bearing container for the LIVE evidence a
future Phase-2 shadow adapter would collect and hand to the Phase-1 pure core (decide()). Proven inert by
tests/test_sss_phase2_shadow_design_v1.py::test_static_guard_no_runtime_imports_phase2_design.

PURITY: standard-library only. No Redis / SQL / HTTP / socket / subprocess / provider SDK; no wall-clock read at
call (all timestamps are supplied); no logging side effect; no thread/task/timer. The snapshot is a frozen
dataclass; the ONLY behaviour is deterministic (de)serialisation + a PURE mapping to the Phase-1 core input.

HONESTY MODEL: every transport field carries FieldProvenance. Provenance records whether the value was
DIRECTLY_OBSERVED (a logged/measured fact) or a JUSTIFIED_INFERENCE (derived, but sound) or UNAVAILABLE. Missing
evidence is represented as UNAVAILABLE — NEVER silently defaulted to a favourable (healthy) value. The July-16
replay uses this to distinguish the historically-logged socket state from the *inferred* heartbeat health that the
observer log did NOT record.
"""
...
# The authoritative governed instrument inventory (architecture_v1 §7/§9). NO substring parsing, NO invented
...
class EvidenceSnapshot:
    ...
    surfaces, then hand it to core.decide() via to_recovery_input(). This class NEVER touches those surfaces — it
    is a pure container. All timestamps are supplied by the caller (no wall-clock read here).
    """
    # ---- identity / provenance ----
    provider: str
    captured_at_utc: str
    ...
    # ---- the PURE mapping to the Phase-1 core input (§12 part 3) ----
    def to_recovery_input(self) -> core.RecoveryDecisionInput:
        """Build the immutable Phase-1 RecoveryDecisionInput from this snapshot. PURE. Raises SnapshotError only
        at the construction boundary (the core's own validation). Does NOT decide anything."""
        evaluated = datetime.datetime.fromisoformat(self.evaluated_at_utc)
        return core.RecoveryDecisionInput(
            evaluated_at_utc=evaluated,
            provider=self.provider,
            contract_version=self.contract_version,
            config_version=self.config_version,
            socket_state=core.SocketState(self.socket_state),
            provider_disconnect_event=self.provider_disconnect_event,
            auth_failure=self.auth_failure,
            heartbeat_available=self.heartbeat_available,
            heartbeat_age_s=self.heartbeat_age_s,
            ...
            heartbeat_soft_horizon_s=self.heartbeat_soft_horizon_s,
            heartbeat_hard_horizon_s=self.heartbeat_hard_horizon_s,
            instruments=tuple(i.to_core() for i in self.instruments),
            limiter=core.LimiterState(
                attempts_in_window=self.limiter_attempts_in_window,
                max_per_window=self.limiter_max_per_window,
                window_seconds=self.limiter_window_seconds,
                available=self.limiter_available,
            ),
            evidence_summary={
...
def snapshot_from_dict(payload: Mapping[str, object]) -> EvidenceSnapshot:
    ...
    dict access, bounded, no eval). Reused by the offline replay harness + tests."""
