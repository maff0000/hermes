...
# Surfaces that MAY legitimately be marked runtime-published in this contract — those with a governed HERMES
# runtime publisher/emitter: instrument_catalog (PR #71/#73), feed_health (PR #74 wiring + PR #76 semantics),
# quote (PR #75 wiring + PR #77 semantics) and tick (LIVE canonical EMITTER, WO-...-LIVE-TICK-PUBLISHER-V1-BUILD).
...
# process is decided by its gate at publish time (see the caller); membership here only permits it. tick is
# emitter-based (per-tick stream loop), not a supervisor runner, but its catalog truth is still gate-driven identically.
RUNTIME_PUBLISHABLE_SURFACES = frozenset({"instrument_catalog", "feed_health", "quote", "tick"})
...
def build_instrument_catalog_contract(*, instrument, generated_at_utc, source_name, snapshot=None,
    ...
    quote, tick); any other entry fails loud. Default None -> nothing runtime-published (the pure builder /
    pre-activation snapshot path is unchanged: surfaces stay CODE_PRESENT_DARK)."""
    _assert_instrument(instrument)
    ...
    # feed_health self-surface runtime-publication (same distinct-truths model as instrument_catalog): when its
    # runtime publisher is active the runtime publisher passes "feed_health" here -> reported RUNTIME_PUBLISHED
    # (not dark), runtime_published=True, consumer_live stays False. quote/tick are NOT runtime-publishable yet.
    ...
    # tick self-surface runtime-publication (same distinct-truths model): runtime-published when the LIVE canonical
    # tick EMITTER is active (the caller passes "tick"), else dark. consumer_live stays False. tick is emitter-based
    # (per-tick stream loop), not a supervisor runner, but the catalog truth is gate-driven identically.
    tick_runtime_published = "tick" in runtime_published
    tick_reported_status = SURFACE_RUNTIME_PUBLISHED if tick_runtime_published else snap["tick"]
    _assert_surface_status(tick_reported_status, "tick:reported")
    ...
    # feed_health (PR #74), quote (PR #75) and tick (LIVE canonical emitter) have governed runtime publishers:
    # runtime-published when their publisher/emitter is active, else dark. `live` is retained as an explicit alias
    # of consumer_live (consumer-cutover), NOT a claim of Redis publication.
    ...
    # (tick_contract_v1) — referenced, NEVER a duplicate hermes:tick:* key. quote/tick runtime-published track their
    # publisher/emitter gate; when off they stay CODE_PRESENT_DARK (key present as a discovery fact, not a live claim).
    ...
    tick_contract = {"key": _safe_key(tickc.canonical_key, CANONICAL_INSTRUMENT), "status": tick_reported_status,
                     "runtime_published": tick_runtime_published, "consumer_live": False, "live": False,
                     "note": "governed tick surface (tick_contract_v1); LIVE canonical emitter writes it when tick gates active"}
    ...
    surfaces = {"candles": {tf: snap["candle_latest"][tf] for tf in SUPPORTED_TIMEFRAMES},
                ...
                "quote": quote_reported_status, "tick": tick_reported_status}
